...
class CommanderLegalityEvaluator {
  ...
  CommanderLegalityEvaluator(CardRepository cardRepository,
  ...
  DeckLegalityResponse evaluate(Deck deck, List<DeckCard> deckCards) {
    ...
    Card partner = commander(deck.getSecondaryCommanderCardId(), "SECONDARY_COMMANDER", violations);
    ...
    validateDeckSize(mainDeck, deck.getCommanderCardId(), deck.getSecondaryCommanderCardId(), violations);
    return new DeckLegalityResponse(violations.isEmpty(), violations);
  }

  private Card commander(Long cardId, String role, List<DeckLegalityResponse.Violation> violations) {
    if (cardId == null) {
    ...
    return cardRepository.findById(cardId).orElseGet(() -> {
      add(violations, role + "_NOT_FOUND", role + " card could not be found.");
      return null;
    });
  }

  private void validateCommanders(Card commander, Card partner,
    ...
    if (partner != null && !eligible(partner) && !paired) {
      add(violations, "SECONDARY_COMMANDER_INELIGIBLE", partner.getName() + " cannot be a commander.");
    }
  ...
  private void validateCommander(Card card, String role,
      List<DeckLegalityResponse.Violation> violations) {
    if (!eligible(card)) {
  ...
  private List<Card> mainDeck(List<DeckCard> deckCards,
      List<DeckLegalityResponse.Violation> violations) {
    return deckCards.stream().filter(card -> card.getDeckSection() == DeckCard.Section.MAIN_DECK)
        .flatMap(card -> cardsFor(card, violations).stream()).toList();
  }

  private List<Card> cardsFor(DeckCard deckCard, List<DeckLegalityResponse.Violation> violations) {
    return cardPrintingRepository.findById(deckCard.getCardPrintingId()).map(CardPrinting::getCard)
        .map(card -> java.util.Collections.nCopies(deckCard.getQuantity(), card)).orElseGet(() -> {
          add(violations, "CARD_NOT_FOUND", "A deck card printing could not be found.");
          return List.of();
        });
  }

  private void validateCards(List<Card> cards, Card commander, Card partner,
      List<DeckLegalityResponse.Violation> violations) {
    Set<String> colorIdentity = colors(commander, partner);
    cards.forEach(card -> validateCard(card, colorIdentity, violations));
    cards.stream().filter(card -> !basicLand(card)).collect(java.util.stream.Collectors.groupingBy(
        Card::getScryfallOracleId, java.util.stream.Collectors.counting())).forEach((oracleId, count) -> {
          if (count > 1) {
            add(violations, "SINGLETON_VIOLATION", "A non-basic card appears more than once.");
          }
        });
  }

  private void validateCard(Card card, Set<String> colorIdentity,
      List<DeckLegalityResponse.Violation> violations) {
    validateLegality(card, violations);
    if (!colorIdentity.containsAll(colors(card))) {
      add(violations, "COLOR_IDENTITY_VIOLATION", card.getName() + " is outside the commander's color identity.");
    }
  }

  private void validateLegality(Card card, List<DeckLegalityResponse.Violation> violations) {
    String status = card.getLegalities().stream()
        .filter(legality -> "commander".equalsIgnoreCase(legality.getFormatCode()))
        .map(legality -> legality.getLegalityStatus()).findFirst().orElse(null);
    if (status == null) {
      add(violations, "COMMANDER_LEGALITY_UNKNOWN", card.getName() + " has no Commander legality data.");
    } else if (!"legal".equalsIgnoreCase(status)) {
      add(violations, "COMMANDER_LEGALITY_INVALID", card.getName() + " is not legal in Commander.");
    }
  }

  private void validateDeckSize(List<Card> cards, Long commander, Long partner,
      List<DeckLegalityResponse.Violation> violations) {
    int commanderCount = (commander == null ? 0 : 1) + (partner == null ? 0 : 1);
    if (cards.size() + commanderCount != 100) {
      add(violations, "DECK_SIZE_INVALID", "A Commander deck must contain exactly 100 cards.");
    }
  }

  private boolean eligible(Card card) {
    return text(card.getTypeLine()).contains("legendary") && text(card.getTypeLine()).contains("creature")
  ...
  private boolean validPair(Card first, Card second) {
    return genericPartner(first) && genericPartner(second)
        ...
        || partnerWith(first, second) || partnerWith(second, first)
        || chooseBackground(first) && background(second) || chooseBackground(second) && background(first)
  ...
  private boolean genericPartner(Card card) {
    return text(card.getOracleText()).lines().map(String::trim)
  ...
  private boolean background(Card card) {
    return text(card.getTypeLine()).contains("legendary enchantment") && has(card, "background");
  }
  ...
  private boolean timeLordDoctor(Card card) {
    return text(card.getTypeLine()).contains("legendary") && has(card, "time lord") && has(card, "doctor");
  }
  ...
  private Set<String> colors(Card... cards) {
    return java.util.Arrays.stream(cards).filter(card -> card != null)
        .flatMap(card -> colors(card).stream()).collect(java.util.stream.Collectors.toSet());
  }
