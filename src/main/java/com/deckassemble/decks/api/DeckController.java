...
public class DeckController {
  ...
  public ResponseEntity<DeckCardResponse> addCard(@PathVariable long deckId,
      @Valid @RequestBody DeckCardAddRequest request) {
    DeckCardResponse added = deckService.addCard(deckId, request);
    return ResponseEntity.created(URI.create("/api/v1/decks/" + deckId + "/cards/" + added.id()))
        .body(added);
  }

  @PatchMapping("/{deckId}/cards/{deckCardId}")
  public DeckCardResponse updateCard(@PathVariable long deckId, @PathVariable long deckCardId,
  ...
  public ResponseEntity<Void> removeCard(@PathVariable long deckId, @PathVariable long deckCardId) {
