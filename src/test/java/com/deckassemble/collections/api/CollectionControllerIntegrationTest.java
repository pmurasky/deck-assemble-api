...
class CollectionControllerIntegrationTest extends AbstractIntegrationTest {
  ...
  private MvcResult assertCreatedCardSummary(String subject, long collectionId, long printingId)
      throws Exception {
    String request = "{\"cardPrintingId\":%d,\"regularQuantity\":1,\"foilQuantity\":0}"
        .formatted(printingId);
    return mockMvc.perform(post("/collections/{collectionId}/cards", collectionId)
            .with(jwt().jwt(jwt -> jwt.subject(subject)))
            .contentType(MediaType.APPLICATION_JSON).content(request))
        .andExpect(status().isCreated()).andExpect(jsonPath("$.card.name").value("Chosen Card"))
        .andExpect(jsonPath("$.card.imageUrl").value("https://img.example/chosen.jpg"))
        .andExpect(jsonPath("$.card.setCode").value("chs")).andReturn();
  }

  private void assertUpdatedCardSummary(String subject, long collectionId, long cardId) throws Exception {
    mockMvc.perform(patch("/collections/{collectionId}/cards/{cardId}", collectionId, cardId)
            .with(jwt().jwt(jwt -> jwt.subject(subject)))
            .contentType(MediaType.APPLICATION_JSON).content("{\"regularQuantity\":3,\"foilQuantity\":1}"))
        .andExpect(status().isOk()).andExpect(jsonPath("$.card.setName").value("Chosen Set"))
  ...
  private void assertListedCardSummary(String subject, long collectionId) throws Exception {
    mockMvc.perform(get("/collections/{collectionId}/cards", collectionId)
            .with(jwt().jwt(jwt -> jwt.subject(subject))))
        .andExpect(status().isOk()).andExpect(jsonPath("$[0].card.oracleId").value("oracle-summary"))
  ...
  private long createPrintingWithLaterSibling(String identifier) {
    ...
    long chosenId = savePrinting(card, "chs", "Chosen Set", "chosen", LocalDate.parse("2020-01-01"));
