...
class DeckControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    @Autowired private com.deckassemble.decks.domain.DeckCardRepository deckCardRepository;
    ...
    void shouldGenerateProxyReplacementPlanWithoutMutatingDeck() throws Exception {
        ...
        long proxyPrinting = createAlternativePrinting("Upg Proxy Draw", "W");
        long ownedPrinting = createAlternativePrinting("Upg Owned Draw", "W");
        createAlternativePrinting("Upg Off Color", "U");
        ownPrinting(subject, ownedPrinting);
        long deckCardId = addCardAndReturnId(subject, deckId, proxyPrinting);
        markProxy(deckId, deckCardId);
        org.mockito.Mockito.when(edhrecClient.fetchCommanderData("upg-cmd-proxies"))
    ...
    void shouldReturnEmptyUpgradePlanWhenNothingToReplace() throws Exception {
        ...
        addCard(subject, deckId, createAlternativePrinting("Upg Settled Draw", "W"), 1);
    ...
    void shouldImproveUnderBudgetCeiling() throws Exception {
        ...
        addCard(subject, deckId, createAlternativePrinting("Upg Budget Draw", "W"), 1);
        long priceyPrinting = createAlternativePrinting("Upg Pricey Draw", "W");
        long cheapPrinting = createAlternativePrinting("Upg Cheap Draw", "W");
        snapshotPrice(priceyPrinting, "10.00");
        snapshotPrice(cheapPrinting, "2.00");
        org.mockito.Mockito.when(edhrecClient.fetchCommanderData("upg-cmd-budget"))
    ...
    void shouldCloseCategoryGapWithMatchingAlternative() throws Exception {
        ...
        org.mockito.Mockito.when(edhrecClient.fetchCommanderData("upg-cmd-gaps"))
                .thenReturn(UPG_GAPS_PAYLOAD);

        mockMvc.perform(
                        post("/decks/{deckId}/upgrade-plans", deckId)
                                .with(jwt().jwt(jwt -> jwt.subject(subject)))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"objective\":\"CLOSE_CATEGORY_GAPS\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.substitutions.length()").value(1))
    ...
    private void markProxy(long deckId, long deckCardId) {
        ...
        deckCard.setOwnershipStatus(com.deckassemble.decks.domain.DeckCard.OwnershipStatus.PROXY);
    ...
    private void snapshotPrice(long printingId, String usd) {
        cardPriceSnapshotRepository.save(
                new com.deckassemble.cards.domain.CardPriceSnapshot(
                        printingId,
                        new com.deckassemble.cards.domain.CardPrice(
                                new BigDecimal(usd), null, null, null),
                        java.time.Instant.now()));
    }
    ...
    private long createUpgradeCommander(String name) {
        ...
        MagicSet set = magicSetRepository.save(new MagicSet("set-" + identifier, setCode, "Upg"));
